export default {
    accounts: {
      ...
      dataExportIncludeTicketInfo: '导出门票信息（包含门票原文）',
      dataImport: '导入',
      moreActions: '更多操作',
      dataActions: '数据操作',
      toolActions: '工具',
      ...
      dataExportConfirmMessage: '导出的数据包含账号凭据、代理信息，勾选门票时还包含门票原文，请按敏感备份妥善保存。',
      ...
      dataImportFile: '数据文件',
      dataImportTicketInfoDetected: '检测到 {count} 条门票；文件中包含原文时，将由服务端原样恢复。',
      ...
      dataImportResultSummary: '代理创建 {proxy_created}，复用 {proxy_reused}，失败 {proxy_failed}；账号创建 {account_created}，失败 {account_failed}；门票恢复 {ticket_restored}；批量配置 {post_import_updated}，配置失败 {post_import_failed}；智能分配 {proxy_assigned}，分配失败 {proxy_assign_failed}',
      dataImportBulkConfigTitle: '导入后批量编辑配置',
      dataImportProfileEditorTitle: '导入配置方案编辑器',
      dataImportBulkConfigHint: '复用批量编辑账号的完整面板，配置只应用到本次成功导入的新账号。',
      dataImportConfigure: '配置项目',
      dataImportSaveBulkConfig: '保存导入配置',
      dataImportBulkConfigReady: '已保存 {count} 个批量配置字段，导入成功后自动应用。',
      dataImportProfilesTitle: '导入配置方案',
      dataImportProfilesHint: '保存多套批量编辑和智能代理参数，点击方案名称即可一键切换。方案保存在当前浏览器，也可导出后在其他浏览器导入。',
      dataImportProfilesListHint: '每套方案同时包含批量编辑和智能代理参数；使用时直接从列表选择。',
      dataImportProfilesEmpty: '暂无配置方案',
      dataImportProfilesEmptyHint: '新建方案后，可在这里一键使用、编辑、复制、导出或删除。',
      dataImportProfileNew: '新建方案',
      dataImportProfileName: '方案名称',
      dataImportProfileUse: '使用',
      dataImportProfileInUse: '使用中',
      dataImportProfileBatchSummary: '批量配置',
      dataImportProfileProxySummary: '智能代理',
      dataImportProfileUpdatedAt: '更新于 {time}',
      dataImportProfileNoBatchConfig: '不修改账号字段',
      dataImportProfileBatchConfigured: '已配置高级账号参数',
      dataImportProfileProxyDisabled: '未开启智能代理',
      dataImportProfileProxyCount: '每账号 {count} 个代理',
      dataImportProfileLatencyTestEnabled: '分配前测速',
      dataImportProfileLatencyTestDisabled: '不重新测速',
      dataImportProfileFastestCandidates: '最快 {count} 个候选',
      dataImportProfileAllHealthyCandidates: '全部健康代理',
      dataImportProfilePreferLatency: '低延迟优先',
      dataImportProfileUniformLatency: '延迟均匀抽选',
      dataImportProfileWeightedLoad: '按负载加权',
      dataImportProfileUnweightedLoad: '不按负载加权',
      dataImportProfileConcurrency: '并发 {value}',
      dataImportProfileLoadFactor: '负载系数 {value}',
      dataImportProfilePriority: '优先级 {value}',
      dataImportProfileRateMultiplier: '倍率 {value}',
      dataImportProfileStatus: '状态 {value}',
      dataImportProfileMainProxy: '主代理 {value}',
      dataImportProfileGroups: '分组 {value} 个',
      dataImportProfileBillingProbe: '倍率探测 {value}',
      dataImportProfileBaseUrl: 'Base URL {value}',
      dataImportProfileModels: '模型限制 {value} 项',
      dataImportProfileErrorCodes: '错误码 {value}',
      dataImportProfileWarmup: '预热拦截 {value}',
      dataImportProfileHeaders: '请求头覆写 {value} 项',
      dataImportProfileCapabilities: '端点能力 {value} 项',
      dataImportProfilePassthrough: 'OpenAI 透传 {value}',
      dataImportProfileLongContext: '长上下文计费 {value}',
      ...
      dataImportProfileCopyName: '{name} 副本',
      dataImportProfileCopied: '配置方案“{name}”已复制',
      dataImportProfileNamePlaceholder: '方案名称，例如：OpenAI 高并发',
      dataImportProfileSaveAs: '另存为新方案',
      dataImportProfileOverwrite: '覆盖当前方案',
      dataImportProfileExport: '导出方案',
      dataImportProfileImport: '导入方案',
      dataImportProfileNameRequired: '请输入方案名称',
      dataImportProfileEmptyConfig: '请先配置批量编辑参数或开启智能代理',
      dataImportProfileNameExists: '已存在同名方案',
      dataImportProfileSaved: '配置方案“{name}”已保存',
      dataImportProfileUpdated: '配置方案“{name}”已更新',
      dataImportProfileApplied: '已切换到配置方案“{name}”',
      dataImportProfileImported: '配置方案“{name}”已导入',
      dataImportProfileImportFailed: '配置方案文件无效或版本不受支持',
      dataImportProfileDeleteConfirm: '确定删除配置方案“{name}”吗？',
      dataImportProfileScopeWarning: '当前 JSON 的平台或账号类型与方案保存时不同，请检查平台专属配置。',
      dataImportProfileStorageFailed: '浏览器无法保存配置方案，请导出方案文件备份。',
      dataImportErrors: '失败详情',
      dataImportSuccess: '导入完成：账号 {account_created}，失败 {account_failed}，门票恢复 {ticket_restored}',
      dataImportCompletedWithErrors: '导入完成但有错误：账号失败 {account_failed}，代理失败 {proxy_failed}，批量配置失败 {post_import_failed}，代理分配失败 {proxy_assign_failed}',
      ...
      smartProxy: {
        title: '一键随机配置代理',
        hint: '先检测代理延迟，再按低延迟和当前账号占用动态加权，让账号之间的代理压力更均匀。',
        proxyCount: '每个账号的并行代理数量',
        proxyCountHint: '第一个作为主代理，其余加入并发代理池。',
        lowLatencyLimit: '低延迟候选数量',
        lowLatencyLimitHint: '0 表示使用全部健康代理；例如 20 表示只从最快的 20 个中分配。',
        testLatency: '分配前重新检测延迟',
        testLatencyHint: '并发测试全部可用代理，失败的代理不会参与分配。',
        preferLowLatency: '优先使用低延迟代理',
        preferLowLatencyHint: '延迟越低，随机抽中的概率越高。',
        weightedByLoad: '按代理负载加权分散',
        weightedByLoadHint: '账号绑定越少、本轮压力越低的代理，分配概率越高。',
        apply: '立即随机分配',
        assigning: '测速并分配中...',
        success: '智能代理分配完成：成功 {success}，失败 {failed}，候选代理 {proxies}',
        failed: '智能代理分配失败'
      },
      ...
      openai: {
        ...
        codexTurnTicket: 'Codex Turn State 门票',
        codexTurnTicketDesc: '显示目标长度、当前观测长度、限流和调度策略。门票原文不会显示。',
        codexTurnTicketMissing: '暂无有效门票，仍允许请求',
        codexTurnTicketReady: '目标票 · 剩余 {time}',
        codexTurnTicketPaused: '未打到目标票 {length}，该模型已暂停',
        codexTicketTargetMode: '目标门票模式',
        codexTicketTargetAuto: '自动识别账号类型',
        codexTicketTargetManual: '手动指定长度',
        codexTicketTargetLength: '目标长度',
        codexTicketTargetLengthInvalid: '目标长度必须在 128 到 2048 之间',
        codexTicketMissingPolicy: '未命中目标票时',
        codexTicketMissingPause: '暂停该账号模型调度',
        codexTicketMissingAllow: '继续调度（不注入门票）',
        codexTicketAutoResolved: '当前自动目标：{length}（{source}）',
        codexTicketSourceBusiness: 'Team/Business',
        codexTicketSourcePersonal: '个人订阅',
        codexTicketSourceManual: '手动配置',
        codexTicketSourceDefault: '系统默认',
        codexTicketStatusDetail: '目标 {target} · 当前 {current} · 缺票策略：{policy}',
        codexTicketPolicyPauseShort: '暂停',
        codexTicketPolicyAllowShort: '继续',
        codexTicketNextProbe: '下次探测 {time}',
        codexTicketTypeNonTarget: '非目标票',
        codexTicketTypeExpired: '目标票已过期',
        codexTicketTypeRateLimited: '上游限流',
        codexTicketTypeQuotaExhausted: '额度耗尽',
        codexTicketTypeError: '探测错误',
        codexTicketTypeDisabled: '模型已停用',
        codexTicketProbeNow: '立即打票',
        codexTicketProbing: '打票中…',
        codexTicketProbeFailed: '手动打票失败',
        codexTicketProbeSuccess: '已拿到目标门票：{model} · {length}/{target}',
        codexTicketProbeNonTarget: '已完成探测，但当前不是目标门票：{model} · {current}/{target}',
        codexTicketProbeNonTargetPreserved: '本次探测不是目标票；{model} 已有的有效门票保持不变',
        codexTicketProbeRateLimited: '探测完成：上游限流，{model} 暂未拿到门票',
        codexTicketProbeError: '探测完成但未拿到门票：{model} · {reason}',
        codexTicketProbeStage: {
          queued: '准备手动取票',
          validating: '校验账号与模型',
          persisting_policy: '保存当前门票策略',
          acquiring_token: '获取账号访问令牌',
          requesting_upstream: '通过打票代理请求上游',
          validating_response: '校验上游门票响应',
          persisting_result: '持久化探测结果',
          refreshing_status: '刷新账号门票状态',
          completed: '手动取票已完成',
          failed: '手动取票失败'
        },
        codexTicketProbeLog: {
          probe_queued: '任务已进入手动取票队列',
          validating_account: '正在读取并校验账号状态',
          validation_passed: '账号与模型校验通过：{model}',
          persisting_policy: '正在保存当前模型策略',
          acquiring_token: '正在获取账号令牌；目标长度 {target_length}',
          token_ready: '账号令牌已就绪',
          sending_probe: '正在通过专用打票代理发送请求',
          response_received: '收到上游响应：HTTP {http_status}，长度 {observed_length}/{target_length}',
          token_unavailable: '账号令牌不可用，正在记录失败状态',
          request_failed: '上游请求失败，正在记录失败状态',
          probe_superseded: '策略已变化，本次旧探测结果已丢弃',
          persisting_observation: '正在保存探测状态：{outcome}',
          persisting_ticket: '已命中目标票，正在持久化长度 {length}',
          refreshing_status: '正在重新读取持久化后的门票状态',
          probe_completed: '任务完成：{outcome}',
          probe_failed: '任务失败；详细原因见页面提示'
        },
      ...
      noProxy: '无代理',
      proxyPool: '并发代理池',
      proxyPoolHint: '业务请求会在主代理和所选代理之间轮询；授权、刷新、检测仍固定使用主代理。',
      proxyPoolEmpty: '未添加额外代理',
      proxyPoolSelected: '已选择 {count} 个额外代理',
      proxyLanes: {
        title: '代理并发泳道',
        hint: '代理只负责分流和显示实时占用；并发、权重、超时和熔断参数统一应用到整个代理池。',
        primary: '主代理',
        unifiedSettings: '代理池统一配置',
        unifiedSettingsHint: '以下参数对主代理和所有并行代理同时生效；备用顺序按代理选择顺序自动生成。',
        accountConcurrency: '账号并发数',
        accountConcurrencyHint: '请在上方“并发数”字段修改，系统会自动平均分配到各泳道。',
        unifiedSummary: '统一 W{weight} · 超时 {timeout}s · {threshold} 次错误熔断 · 冷却 {cooldown}s',
        maxConcurrency: '最大并发',
        weight: '权重',
        timeout: '超时（秒）',
        circuitThreshold: '连续错误熔断',
