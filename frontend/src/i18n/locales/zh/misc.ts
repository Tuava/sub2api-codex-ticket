export default {
  ...
  version: {
    ...
    githubRateLimited: 'GitHub API 请求额度已用尽，系统已尝试备用发布源。请稍后重试，或在部署环境中配置 UPDATE_GITHUB_TOKEN。',
    githubRateLimitedUntil: 'GitHub API 请求额度已用尽，备用发布源也不可用。请在 {time} 后重试，或配置 UPDATE_GITHUB_TOKEN。',
    githubUnavailable: 'GitHub API 与备用发布源均不可用：{message}',
