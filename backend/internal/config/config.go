...
type UpdateConfig struct {
	...
	// Repository is the GitHub owner/repository used for update and rollback releases.
	// Empty selects the customized repository for -tuava builds and upstream otherwise.
	Repository string `mapstructure:"repository"`
	// DockerImage is shown in rollback instructions. Empty is derived from Repository.
...
type OpenAICodexTicketConfig struct {
	...
	ModelPolicies                map[string]OpenAICodexTicketModelPolicy `mapstructure:"model_policies" json:"model_policies"`
}

type OpenAICodexTicketModelPolicy struct {
	Enabled       bool   `mapstructure:"enabled" json:"enabled"`
	TargetMode    string `mapstructure:"target_mode" json:"target_mode"`
	TargetLength  int    `mapstructure:"target_length" json:"target_length"`
	MissingPolicy string `mapstructure:"missing_policy" json:"missing_policy"`
}
...
func setDefaults() {
	...
	viper.SetDefault("gateway.openai_codex_ticket.harvest_probe_interval_seconds", 1800)
...
func setEnvReachableDefaults() {
	...
	viper.SetDefault("update.docker_image", "")
